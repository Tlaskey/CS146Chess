...
public class Pawn extends Piece {
	...
	public void moves(Board b) {
		...
		if (row == 0 || row == 7) {
			...
			// Right, diagonal
			if (r > 0 && r < 7 && c < 7) {
				//System.out.println(row + "" + column + " is a " + name);
				r = r - c1;
				c = c + 1;
				m = Integer.toString(r) + Integer.toString(c);// Move
				if (c < 7 && team != b.getPiece(r, c).getTeam() && b.getPiece(r, c).getName() != null) {
					moves[i] = m + " This move kills the "
							+ b.getPiece(r, c).getName() + " of the opponent";
					i++;
				}
