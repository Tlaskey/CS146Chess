...
public class Knight extends Piece {
	...
	public Knight(boolean t, String n, boolean m, int x, int y, boolean d, boolean e) {
		super(t, n, m, x, y, d, e);
		limit = 18;
	...
	public void moves(Board b) {
		...
		if (r > 1 && c >= 1 && b.getPiece(r - 2, c - 1).getName() != null) {
			...
			if(team != b.getPiece(r - 2, c - 1).getTeam()){
				moves[i] = m;
				i++;
			}
			String by = Integer.toString(row) + Integer.toString(column);
		...
		if (r + 2 <= 7 && c + 1 <= 7 && b.getPiece(r + 2, c + 1).getName() != null) {
			m = Integer.toString(r + 2) + Integer.toString(c + 1)
			+ " This move kills the " + b.getPiece(r + 2, c + 1).getName() + " of the opponent";
			if (team != b.getPiece(r+2, c+1).getTeam()) {
				moves[i] = m;
				i++;
			}
